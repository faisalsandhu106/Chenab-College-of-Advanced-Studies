...
const ComingEventSection = () => {
    const { isEventLoading, futerEvent } = UseCourseContext();

    //* Loading Animation
    if (isEventLoading || futerEvent.length === 0) {
        return (
    ...
    return (
        <>
            ...
            <motion.div
                ...
                viewport={{ once: true }}
                className='grid xs:grid-cols-1 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 justify-items-center pt-6 xs:px-1 md:px-3 lg:px-5 xl:px-12 md:gap-x-7 xs:gap-y-10 md:gap-y-12 w-full'>
                {
                    futerEvent.map((curElem) => {
                        const { _id, eventTitle, longEventDetails, imageUrl, eventFee, category } = curElem
                        return (
                            <motion.div variants={DownToUpAnimation} key={_id} className='w-full h-full'>
                                <div className='Card-box flex flex-col justify-between w-full h-full rounded-md overflow-hidden'>
                                    <div className='flex flex-col justify-between gap-y-3 w-full capitalize'>
                                        <figure className='flex items-start xs:h-[170px] md:h-[135px] lg:h-[145px] xl:h-[155px] overflow-hidden'>
                                            <img className='w-full h-auto bg-cover' src={imageUrl[0]} alt="course_img" />
                                        </figure>
                                        <div className='flex flex-col gap-y-2 px-3'>
                                            ...
                                            <div className='flex flex-col font-medium capitalize'>
                                                ...
                                                <div className='title-underline mt-1 w-[40px]'></div>
                                            </div>
                                            <p className='xs:text-[0.9em] md:text-[0.78em] lg:text-[0.85em] xl:text-[0.9em]'>
                                                {longEventDetails.length > 90 ? longEventDetails.slice(0, 90) + '...' : longEventDetails}
                                            </p>
                                        </div>
                                    </div>
                                    <div className='flex items-center justify-end px-4 pt-7 pb-4 w-full'>
                                        <NavLink to={`/event/${_id}/singlePage`} className=''>
