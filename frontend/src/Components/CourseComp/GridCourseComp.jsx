...
const GridCourseComp = ({ isLoading, Allcourses, filterCourse }) => {

    //* Loading Animate
    if (isLoading || Allcourses.length === 0) {
        return (
            <div className='grid xs:grid-cols-1 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 justify-items-center md:gap-x-4 lg:gap-x-6 xl:gap-x-6 xs:gap-y-10'>
                ...
                filterCourse.length === 0 ? (
                    ...
                    (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className='grid xs:grid-cols-1 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 justify-items-center md:gap-x-7 xs:gap-y-10 md:gap-y-12'>
                            {
                                filterCourse.map((item, index) => {
                                    const { _id, image, title, courseFee, description, level } = item;
