...
const AllStudentAddmission = () => {
    const [search, setSearch] = useState()
    const { isStudentAddmissionLoading, AllStudentAddmission, deleteAdmissionForm } = UseAdminContext();
    ...
    const filterData = AllStudentAddmission.filter((e) => searchCourse(e));
    ...
    return (
        <>
            ...
            <div className='xs:pt-5 xl:pt-8 xs:pb-10 md:pb-12 w-full'>
                <div className='flex flex-col items-center xs:text-[1.6em] font-bold capitalize text-center'>
                    <h1>
                        Student Addmissions
                    </h1>
            ...
            {/* Filter Section */}
            <div className='grid xs:grid-cols-1 md:grid-cols-2 md:px-5 lg:px-7 xl:px-8 2xl:px-12 w-full'>
                <form onSubmit={(e) => e.preventDefault()} className='flex items-center gap-x-1 md:w-[270px] lg:w-[300px] xl:w-[350px]'>
                    ...
                    <input
                        ...
                        type="text"
                        placeholder='search a Addmission'
                ...
                <div className='flex items-center justify-end gap-x-1 pr-[2px] pt-2 xs:text-[0.96em] md:text-[0.9em]'>
                    {`${filterData.length > 0 ? filterData.length + " Student Addmission" : 'Addmission Not Found'} `}
                    <PiStudentBold />
                </div>
            </div>
            ...
            {/* ----------Student Reviews API---------- */}
            <AdmissionCard
                isStudentAddmissionLoading={isStudentAddmissionLoading}
                AllStudentAddmission={AllStudentAddmission}
                filterData={filterData}
                deleteAdmissionForm={deleteAdmissionForm} />
        </>
    )
}

export default AllStudentAddmission